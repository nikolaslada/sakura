...
class NumberedColumns {
	...
	* @param array|DibiRow Pass an array which are indexed by number.
	* @return void
	...
	*/
	public function setList ($array) {
		...
		if(!is_array($array) and !$array instanceof DibiRow) throw new InvalidArgumentException('The $array argument is not array or DibiRow object.');
	...
	*/
	...
	/**
	* Add columns into select clause of DibiFluent object.
	* @param DibiFluent Put your DibiFluent instance.
	* @param int|NULL Pass integer as left endpoint of closed interval. If NULL is passed, $from will be equal $this->min.
	* @param int|NULL Pass integer as right endpoint of closed interval. If NULL is passed, $to will be equal $this->max.
	* @return DibiFluent
	* @throws DomainException
	...
	* @throws LogicException
	*/
	public function setSelect ($DibiFluent, $from, $to) {
		...
		for( ; $from <= $to; $from++) {
			$DibiFluent->select("`{$this->alias}`.`{$this->name}$from`");
		}
	...
	/**
	* Add columns into order by clause of DibiFluent object.
	* @param DibiFluent Put your DibiFluent instance.
	* @param int|NULL Pass integer as left endpoint of closed interval. If NULL is passed, $from will be equal $this->min.
	* @param int|NULL Pass integer as right endpoint of closed interval. If NULL is passed, $to will be equal $this->max.
	...
	* @throws LogicException
	*/
	public function setOrderBy ($DibiFluent, $from, $to, $asc) {
		...
		if($asc) {
			for( ; $from <= $to; $from++) {
				$DibiFluent->orderBy("`{$this->alias}`.`{$this->name}$from` ASC");
			}
		} else {
			for( ; $from <= $to; $from++) {
				$DibiFluent->orderBy("`{$this->alias}`.`{$this->name}$from` DESC");
			}
		}
	...
	/**
	* Add conditions of where clause into DibiFluent object.
	* @see $this->checkList()
	* @param DibiFluent Put your DibiFluent instance.
	...
	* @throws DomainException
	*/
	public function setWhere ($DibiFluent, $from, $to, $comparisonOperator, $useAnd) {
			...
			if(is_null($this->type)) {
				foreach($this->list as $key => $value) {
					if($from <= $key and $key <= $to) {
						...
						if($useAnd or $i == 0) $DibiFluent->where("`{$this->alias}`.`{$this->name}$key` $comparisonOperator %$type", $value);
						else $DibiFluent->or("`{$this->alias}`.`{$this->name}$key` $comparisonOperator %$type", $value);
				...
				foreach($this->list as $key => $value) {
					if($from <= $key and $key <= $to) {
						if($useAnd or $i == 0) $DibiFluent->where("`{$this->alias}`.`{$this->name}$key` $comparisonOperator %{$this->type}", $value);
						else $DibiFluent->or("`{$this->alias}`.`{$this->name}$key` $comparisonOperator %{$this->type}", $value);
	...
	/**
	* Add columns into set clause of DibiFluent object. It only set column to other column. For example LVL2 = LVL3.
	* @param DibiFluent Put your DibiFluent instance.
	* @param int Pass integer as left endpoint of closed interval.
	* @param int|NULL Pass integer as right endpoint of closed interval. If NULL is passed, $to will be equal $this->max.
	...
	* @throws LogicException
	*/
	public function setUpdate ($DibiFluent, $from, $to, $movement, $add) {
		...
		if($movement >= 0) {
			...
			while($from <= $to) {
				...
				$DibiFluent->set("`{$this->alias}`.`{$this->name}$to` = `{$this->alias}`.`{$this->name}$right`$add");
				$to--;
				$right--;
			}
			...
			while($from <= $to) {
				$DibiFluent->set("`{$this->alias}`.`{$this->name}$from` = `{$this->alias}`.`{$this->name}$right`$add");
				$from++;
				$right++;
			}
		}
	...
	/**
	* Add columns into set clause of DibiFluent object. It sets values to numbered columns.
	* @param DibiFluent Put your DibiFluent instance.
	...
	*/
	public function setUpdateValues ($DibiFluent, $values, $from, $to) {
		...
		if(is_null($this->type)) {
			foreach($values as $k => $v) {
				...
				if($k <= $to and $k >= $from) $DibiFluent->set("`{$this->alias}`.`{$this->name}$k` = %$type", $v);
				else throw new OutOfRangeException("One or more keys from \$values is out of defined range <$from; $to>! Key: $k.");
			}
		} else {
			foreach($values as $k => $v) {
				if($k <= $to and $k >= $from) $DibiFluent->set("`{$this->alias}`.`{$this->name}$k` = %{$this->type}", $v);
				else throw new OutOfRangeException("One or more keys from \$values is out of defined range <$from; $to>! Key: $k.");
			}
		}
