...
class Table {
	...
	*/
	public function __construct($name, $columns, $alias = '', $enabledTransaction = True) {

		if(!is_string($name) or empty($name)) throw new InvalidArgumentException('Class Table: parameter $name must be a non-empty string! Type of $name is ' . gettype($name) . '.');
		if(!is_array($columns)) throw new InvalidArgumentException('Class Table: parameter $columns must be an array! Type of $columns is ' . gettype($columns) . '.');
		if(!is_string($alias)) throw new InvalidArgumentException('Class Table: parameter $alias must be a string! Type of $alias is ' . gettype($alias) . '.');

		if(empty($alias)) $this->alias = substr($name, 0, 1) . substr(md5($name), 0, 2);
	...
	*/
	public function addNumbered ($key, $name, $max, $min = 1, $type = NULL) {
		...
		if(empty($key)) throw new InvalidArgumentException("\$key must not be empty; value: $key, type: " . gettype($key) . '.');
	...
	*/
	public function setActiveNumbered ($key) {
		...
		if(!isset($this->numbered[$key])) throw new DomainException("There is no $key key/identifier in the \$this->numbered collection of NumberedColumns instances.");
	...
	*/
	public function getKeysOfNumbered () {
		...
		if(!isset($this->numbered)) throw new LogicException('There is no $this->numbered. Call $this->addNumbered() before.');
	...
	*/
	public function setNumberedUpdate ($from, $to, $movement = 1, $add = 0, $DibiFluent = NULL) {
		...
		if(is_null($DibiFluent)) $DibiFluent = dibi::command()->update("`{$this->name}` AS `{$this->alias}`");
	...
	*/
	public function detectColumns () {
		...
		$fields = dibi::query("SELECT * FROM `{$this->name}` %ofs", 0, "%lmt", 1);
		if(count($fields) == 0) throw new SakuraNoRowException("There is no row in table `{$this->name}`.");
		...
		foreach($numberedSet as $findName => $numbered) {
			foreach($columns as $key => $column) {
				...
				if(is_numeric($numberMaybe)) {
					...
					if($number >= $numbered['min'] and $number <= $numbered['max']) unset($columns[$key]);
				}
			}
	...
	*/
	private function detectNumbered ($columns) {
		...
		if(empty($numbered)) return False;
	...
	*/
	public function checkColumns ($columns) {
		...
		if(!is_array($columns)) throw new InvalidArgumentException('"$columns" must be array; current type is ' . gettype($columns) . '.');
	...
	*/
	public function setLimitOffset ($limit = 0, $offset = 0) {
		...
		if(!is_int($limit) or !is_int($offset)) throw new InvalidArgumentException('Both $limit and $offset must be integers! Types: ' . gettype($limit) . ', ' . gettype($offset) . '.');
		if($limit < 0 or $offset < 0) throw new DomainException("Variables \$limit and \$offset must be both in greater than or equal to 0. Values: $limit, $offset.");
	...
	*/
	public function increaseOffset ($movement = 1) {
		...
		if(!is_int($movement)) throw new InvalidArgumentException('Variable $movement must be an integer! Type: ' . gettype($movement) . '.');
		if($movement < 0) throw new DomainException('The value of $movement must be from 0 and should not exceed in total with $this->offset 18446744073709551615. Value: ' . $movement . '; type: ' . gettype($movement) . '.');
	...
	*/
	public function getAllColumns () {
		...
		if(!isset($this->numbered)) return $this->columns;
	...
	*/
	public function getCheckedColumns () {
		...
		if(!isset($this->checkedColumns)) throw new LogicException('$this->checkedColumns was not set yet. Call $this->checkColumns() before.');
