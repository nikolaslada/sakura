...
class OrderTree extends GeneralTree implements ITreeDriver {
    ...
    */
    public function setOrder ($node) {
        ...
        if (!isset($node[$this->order])) {
            throw new \Sakura\SakuraBadColumnException('Column "order" must be set!', 0, '', array('order' => $this->order), 'notfound');
        }

        if (!is_numeric($node[$this->order])) {
            throw new \DomainException("Value under '{$this->order}' key must be integer or numeric string! Values: " . $node[$this->order]);
        }

        if(isset($node['endOrder'])) {
            if (!is_numeric($node['endOrder'])) {
                throw new \DomainException("Value under 'endOrder' key must be integer or numeric string! Values: " . $node['endOrder']);
            }
    ...
    */
    public function chooseTarget ($id) {
        ...
        $targetPDO = $this->connection->fetch("SELECT `{$this->parent}`, `{$this->depth}`, `{$this->order}` FROM `$table` WHERE `{$this->id}` = %i", $id);
    ...
    */
    public function chooseTargetAsChild ($id, $position = 0) {
        ...
        if($position) {
            $targetPDO = $this->connection->fetch("SELECT $id AS `{$this->parent}`, `{$this->depth}`+1 AS `{$this->depth}`, `{$this->order}`+$position AS `{$this->order}`" .
            ...
            $depthOrder = $this->connection->fetch("SELECT `{$this->depth}`+1 AS `depth`, `{$this->order}`+1 AS `order` FROM `$table` WHERE `{$this->id}` = %i", $id);
    ...
    */
    public function selectOrder ($endOrder = False, $depth = False) {
        ...
        $node = $this->connection->fetch($sql, $id);
        ...
        if ($depth) {
            $this->node[$this->depth] = $node[$this->depth];
        }
    ...
    */
    private function selectEndOrder ($node) {
        ...
        $result = $this->connection->fetch("SELECT `{$this->id}`, `{$this->order}`+1 AS `endOrder` FROM `" . $this->table->getName() .
            "` WHERE `{$this->order}` >= %i", $node['endOrder'], "AND `{$this->parent}` = %i", $node[$this->id], "ORDER BY %by", $this->order, "DESC", "%ofs", 0, "%lmt", 1);

        if (!$result) {
            return $node['endOrder'];
        } else {
            return $this->selectEndOrder($result);
        }
    }
    ...
    */
    public function selectTree ($Fluent = NULL) {
        ...
        if (empty($this->treeFromDB)) {
            throw new \Sakura\SakuraNoRowReturnedException('There is no tree in table!', 0, $table, $this->connection->$sql);
        }
    }
    ...
    */
    public function selectBranch ($Fluent = NULL) {
        ...
        $this->treeFromDB = $Fluent->where("`$alias`.`{$this->order}` >= %i", $orders[$this->order])
    ...
    */
    public function selectDepth () {
        ...
        $depth = $this->connection->fetchSingle("SELECT `{$this->depth}` FROM `" . $this->table->getName() . "` WHERE `{$this->id}` = %i", $this->getId());
        if ($depth === False) {
    ...
    */
    ...
    public function insertNode () {
        ...
        try {
            ...
            if ($PDO[$this->parent] xor $PDO[$this->depth]) {
                throw new \Sakura\SakuraNotSupportedException("Parent and depth values must be both 0 or both greater than 0.");
            }

            if($PDO[$this->parent] and $PDO[$this->depth]) {
                $this->connection->query("UPDATE `" . $this->table->getName() . "` SET `{$this->order}`=`{$this->order}`+1 WHERE `{$this->order}`>=%i", $PDO[$this->order]);
            }

            if(!$PDO[$this->parent] and !$PDO[$this->depth]) {
                if ($PDO[$this->order] != 1) {
                    throw new \Sakura\SakuraNotSupportedException('Order value is ' . $PDO[$this->order] . ' and should be 1!');
                } else {
                    $this->connection->query("UPDATE `" . $this->table->getName() . "` SET `{$this->depth}`=`{$this->depth}`+1, `{$this->order}`=`{$this->order}`+1");
                }
            }
            ...
            if (is_array($node)) {
                $values = array_merge($node, $PDO);
            } else {
                $values = array_merge($node->toArray(), $PDO);
            }
            ...
            if(!$PDO[$this->parent] and !$PDO[$this->depth]) {
                $this->connection->query("UPDATE `" . $this->table->getName() . "` SET `{$this->parent}`=%i", $this->connection->getInsertId(), "WHERE `{$this->order}` = 2");
            }
    ...
    */
    public function deleteNode () {
        ...
        try {
            ...
            $node = $this->connection->fetch("SELECT `{$this->parent}`, `{$this->order}` FROM `$table` WHERE `{$this->id}` = %i", $id);
            if (!$node) {
                throw new \Sakura\SakuraRowNotFoundException('Row not found!', 0, $table, $id);
            }
            ...
            if($node[$this->order] < $node['endOrder']) {
                $this->connection->query("UPDATE `$table` SET `{$this->parent}` = %i", $node[$this->parent], "WHERE `{$this->parent}` = %i", $this->getId());
                $this->connection->query("UPDATE `$table` SET `{$this->depth}` = `{$this->depth}`-1 WHERE `{$this->order}` > %i", $node[$this->order],
                    "AND `{$this->order}` <= %i", $node['endOrder']);
            }

            $this->connection->query("UPDATE `$table` SET `{$this->order}` = `{$this->order}`-1 WHERE `{$this->order}` > %i", $node[$this->order]);
    ...
    */
    public function updateBranch () {
        ...
        try {
            ...
            if (empty($targetPDO[$this->parent])) {
            ...
            if($node[$this->order] <= $targetPDO[$this->order] and $targetPDO['endOrder'] <= $node['endOrder']) {
                ...
                throw new \Sakura\SakuraBadColumnException($msg, $code = 0, $table, $badCols, $type = 'subnode');
            }

            $this->connection->query("UPDATE `$table` SET `{$this->order}` = `{$this->order}`+$move WHERE `{$this->order}` >= %i", $targetPDO[$this->order]);
            ...
            $this->connection->query("UPDATE `$table` SET `{$this->parent}` = %i", $targetPDO[$this->parent], " WHERE `{$this->id}` = %i", $id);
            ...
            if (0 < $depthDiff) {
                $sql[0] .= "`{$this->depth}` = `{$this->depth}`+$depthDiff, ";
            } elseif ($depthDiff < 0) {
                $sql[0] .= "`{$this->depth}` = `{$this->depth}`$depthDiff, ";
             }

            if(0 < $diff) {
                $sql[0] .= "`{$this->order}` = `{$this->order}`+$diff ";
            } elseif ($diff < 0) {
                $sql[0] .= "`{$this->order}` = `{$this->order}`$diff ";
            }
            ...
            $this->connection->query("UPDATE `$table` SET `{$this->order}` = `{$this->order}`-%i", $move, "WHERE `{$this->order}` >= %i", $node['endOrder']);
    ...
    public function selectTreeSqlFactory ($selectId = False) {
        ...
        $alias = $this->table->getAlias();
        return $this->connection->select("`$alias`.`{$this->id}`, `$alias`.`{$this->parent}`, `$alias`.`{$this->depth}`")
    ...
    */
    public function getOrder ($endOrder = False, $depth = False) {
        ...
        if (!isset($this->node[$this->order])) {
            throw new \Sakura\SakuraBadColumnException('Column not found in $this->node.', 0, '', array('order' => $this->order), 'notfound');
        }

        if (!$endOrder and ! $depth) {
            return $this->node[$this->order];
        }

        if($endOrder) {
            if (!isset($this->node['endOrder'])) {
                throw new \Sakura\SakuraBadColumnException('Column not found in $this->node.', 0, '', array('endOrder' => 'endOrder'), 'notfound');
            }
        }

        if($depth) {
            if (!isset($this->node[$this->depth])) {
                throw new \Sakura\SakuraBadColumnException('Column not found in $this->node.', 0, '', array('depth' => $this->depth), 'notfound');
            }
        }
    ...
    */
    public function getWhereTo () {
        ...
        if(!isset($this->whereTo[$this->parent]) or !isset($this->whereTo[$this->depth]) or !isset($this->whereTo[$this->order])) {
            ...
            throw new \Sakura\SakuraBadColumnException($msg, 0, '', array('parent' => $this->parent, 'depth' => $this->depth, 'order' => $this->order), 'notset');
        }
