...
class Tree {
    ...
    */
    public function __construct(Connection $connection, Table $table, $driver = '', $checkStructure = True, $levelName = '') {

        if(!is_string($driver)) {
            throw new \InvalidArgumentException('Parameter $driver must be a string! Type: ' . gettype($driver) . '.');
        }
        ...
        if ($driver == '') {
            foreach ($driversWithoutNC as $driver) {
                if ($this->choose($driver, $columns)) {
        ...
        if($driver == 'l') {
            $this->checkLevelStructure($columns, $levelName);
        } else {
        ...
        if (!isset($this->activeDriver)) {
            throw new \Sakura\SakuraNotSupportedException('Cannot initialize a tree driver.');
        }
    }
    ...
    public function __call ($method, $args) {
        ...
        if ($method == '') {
            throw new \LogicException("Cannot call unnamed method of class 'Tree' or Tree's child.");
        }

        $Reflection = new \ReflectionMethod( get_class($this->usedDrivers[$this->activeDriver]), $method );

        if (!$Reflection->isPublic()) {
            throw new \LogicException("Cannot call non-public method of class 'Tree' or Tree's child.");
        }
    ...
    */
    private function choose ($driver, $columns, $levelName = '') {
        ...
        if ($driver == 't') {
            return $this->checkTraversalStructure($columns);
        } elseif ($driver == 'o') {
            return $this->checkOrderStructure($columns);
        } elseif ($driver == 'p') {
            return $this->checkParentStructure($columns);
        } elseif ($driver == 'l') {
            return $this->checkLevelStructure($columns, $levelName);
        }

        throw new \Sakura\SakuraNotImplementedException("Parameter \$driver: there is no '$driver' driver!");
    }
    ...
    */
    private function initialize ($driver, $id = '') {
        ...
        if ($driver == 't') {
            $this->usedDrivers[$driver] = new TraversalTree($this->connection, $this->table);
        } elseif ($driver == 'o') {
            $this->usedDrivers[$driver] = new OrderTree($this->connection, $this->table);
        } elseif ($driver == 'p') {
            $this->usedDrivers[$driver] = new ParentTree($this->connection, $this->table);
        } elseif ($driver == 'l' and ! empty($id)) {
            $this->usedDrivers[$driver] = new LevelTree($this->connection, $this->table, $id);
        } else {
            throw new \Sakura\SakuraNotImplementedException("Parameter \$driver: there is no '$driver' driver!");
        }
    ...
    */
    private function checkLevelStructure ($columns, $levelName = '') {
        ...
        if (!in_array('id', $columns) and ! in_array($this->table->getName() . '_id', $columns)) {
            throw new \Sakura\SakuraNotSupportedException("There is no 'id' or '" . $this->table->getName() . "_id'.");
        }

        $numberedKeys = $this->table->getKeysOfNumbered();
        if ($levelName and count($numberedKeys) > 1) {
            if (!in_array($levelName, $numberedKeys)) {
                throw new \OutOfBoundsException("No \$levelName $levelName found in collection of NumberedColumns class.");
            }

        $this->table->setActiveNumbered($levelName);
        $this->initialize('l', $levelName);
        } else {
            $this->initialize('l', $this->table->getNameOfNumbered());
        }
    }
