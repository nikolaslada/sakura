...
class LevelTree extends GeneralTree implements ITreeDriver {
	...
	*/
	public function setLevels ($array) {
		...
		if(!is_array($array) and !$array instanceof DibiRow) throw new InvalidArgumentException('Parameter $array must be an array or DibiRow object. Type of value is ' . gettype($array) . '.');

		$addName = $this->table->getNameOfNumbered();
		foreach($array as $k => $v) {
			if(!is_int($v)) throw new DomainException('A value of passed $array is not integer. Current type is ' . gettype($v) . '.');

			if(is_int($k)) $this->node[($addName . $k)] = $v;
			elseif(is_string($k)) $this->node[$k] = $v;
			else throw new DomainException('A key of passed $array is not integer or string. Current type is ' . gettype($k) . '.');
		}
	}
	...
	*/
	public function chooseTargetAsChild ($id, $position = 0) {
		...
		if($id < 1 or $position < 0) throw new DomainException("Pass \$id > 0 and \$position >= 0 together. Values: $id; $position.");
		...
		$DibiFluent = dibi::select("MAX(`$alias`.`$columnPrefix$firstNull`)+1")
			->from("`" . $this->table->getName() . "` AS `$alias`");

		$DibiFluent = $this->table->setNumberedWhere($DibiFluent, $this->table->getMinNumbered(), $nodesPositions[0]);
		$max = $DibiFluent->fetchSingle();

		if(!$max) throw new SakuraNoRowReturnedException("Look at query.", 0, $this->table->getName(), dibi::$sql);
	...
	private function getNodesPositions ($id) {
		...
		$DibiFluent = $this->table->setNumberedSelect($this->baseSqlFactory(), $from = NULL, $to = NULL);
		$node = $DibiFluent->where("`" . $this->table->getAlias() . "`.`{$this->id}` = %i", $id)->fetch();
	...
	private function getLastNonEmptyLevel ($startLevel) {
		...
		$DibiFluent = $this->baseSqlFactory();
		$DibiFluent->select("1");
		$DibiFluent = $this->table->setNumberedWhere($DibiFluent, $this->table->getMinNumbered(), $startLevel++);

		$max = $this->table->getMaxNumbered();
		if($startLevel > $max) throw new SakuraBadColumnException("There is no level $startLevel!");

		for( ; $startLevel <= $max; $startLevel++) {
			$clone = clone $DibiFluent;
			$clone->where("`" . $this->table->getAlias() . "`.`" . $this->table->getNameOfNumbered() . "$startLevel` = %i", 1);

			if(!$clone->fetchSingle()) return $startLevel - 1;
		}
	...
	* @param DibiFluent|NULL You can pass your DibiFluent instance, then it will save result to $this->node property. Or you can skip it, NULL will be passed and result will be passed into $this->setLevels().
	* @return void
	* @throws SakuraRowNotFoundException
	*/
	public function selectNode ($DibiFluent = NULL) {

		if(!$DibiFluent instanceof DibiFluent) {
			$setLevels = True;
			$DibiFluent = $this->baseSqlFactory();
			$DibiFluent = $this->table->setNumberedSelect($DibiFluent, $from = NULL, $to = NULL);
		}
		...
		$this->node = $node;
		if(isset($setLevels)) $this->setLevels($node);
	}
	...
	*/
	protected function selectNodeWhereTo ($nodeID, $position = 0) {
		...
		if(!is_int($nodeID)) throw new InvalidArgumentException('Type of "$nodeID" must be integer. Type of "$nodeID" is ' . gettype($nodeID) . '.');

		$DibiFluent = $this->baseSqlFactory();
		$DibiFluent = $this->table->setNumberedSelect($DibiFluent, $from = NULL, $to = NULL);
		$alias = $this->table->getAlias();
		$node = $DibiFluent->where("`$alias`.`{$this->id}` = %i", $nodeID)->fetch();
		...
		if(!$position) {
			...
			$DibiFluent = dibi::select("SELECT MAX(`$columnName$firstNull`)+1");
			$DibiFluent = $this->setNumberedWhere($DibiFluent, 1, $lastNotNull);
			$this->whereTo[$firstNull] = $DibiFluent->fetchSingle();
			$this->isMax = True;
		} elseif(is_int($position)) $this->whereTo[$firstNull] = $position;
		else throw new InvalidArgumentException('Type of "$position" must be integer. Or pass empty value. Type of "$position" is ' . gettype($position) . '.');
	}
	...
	* @param DibiFluent You can pass your edited DibiFluent instance from $this->selectTreeSqlFactory(). You can skip it.
	* @return void
	...
	*/
	public function selectTree ($DibiFluent = NULL) {
		...
		if(empty($this->treeFromDB)) throw new SakuraNoRowReturnedException("There is no tree in table.", 0, $this->table->getName(), dibi::$sql);
	}
	...
	* @param DibiFluent You can pass your edited DibiFluent instance from $this->selectBranchSqlFactory(). You can skip it too.
	* @return void
	* @throws SakuraRowNotFoundException
	*/
	public function selectBranch ($DibiFluent = NULL) {
		...
		$alias = $this->table->getAlias();
		$rootDibiFluent = dibi::select(False)
			->from("`" . $this->table->getName() . "` AS `$alias`")
			->where("`$alias`.`{$this->id}` = %i", $this->getId());
		$rootDibiFluent = $this->table->setNumberedSelect($rootDibiFluent, $from = NULL, $to = NULL);
		$root = $rootDibiFluent->fetch();
		if(!$root) throw new SakuraRowNotFoundException('Row not found.', 0, $this->table->getName(), $this->getId());

		$lastNotNull = $this->getPositionOfFirstNull($root) - 1;
		if(!$DibiFluent instanceof DibiFluent) $DibiFluent = $this->selectBranchSqlFactory();

		$DibiFluent = $this->table->setNumberedSelect($DibiFluent, $lastNotNull, $to = NULL);
		$this->table->setNumberedList($root);
		$DibiFluent = $this->table->setNumberedWhere($DibiFluent, $this->table->getMinNumbered(), $lastNotNull);
		$this->treeFromDB = $DibiFluent->fetchAll();
	}
	...
	* @param DibiFluent Pass your DibiFluent object.
	* @return void
	...
	*/
	public function selectPath ($includingNode = True, $fromRoot = True, $DibiFluent = NULL) {
		...
		$firstNodeDF = $this->table->setNumberedSelect(clone $DibiFluent, NULL, NULL);
		...
		if($includingNode) $this->list[$currentColumn] = $node;

		if(count($this->list) + (!$includingNode) != $currentColumn) throw new SakuraException('Property $this->list has got less records than should be contain!');
		if($fromRoot) ksort($this->list);
	}
	...
	private function getPositionOfFirstNull ($node) {
		...
		foreach($node as $key => $value) {
			if(empty($value)) {
				...
				if($position != $key) {
					if(!is_numeric($position)) throw new SakuraBadColumnException("Passed \$node's key '$key' is not valid, '$position' is not numeric.");
	...
	*/
	public function selectNumOfSubnodes () {
		...
		if(is_null($currentColumn)) $lastNotNull = $this->table->getMaxNumbered();
		else $lastNotNull = $currentColumn - 1;

		$alias = $this->table->getAlias();
		$forCount = $this->table->getNumberedList($lastNotNull, $lastNotNull);
		$DibiFluent = dibi::select("COUNT(`$alias`.`" . $this->table->getNameOfNumbered() . key($forCount) . "`)")
			->from("`" . $this->table->getName() . "` AS `$alias`");
		$DibiFluent = $this->table->setNumberedWhere($DibiFluent, $this->table->getMinNumbered(), $lastNotNull);
		$subnodesWithRoot = $DibiFluent->fetchSingle();
		if($subnodesWithRoot === False) throw new SakuraNoRowReturnedException('No row returned!', 0, $this->table->getName(), dibi::$sql);
	...
	*/
	public function insertNode () {
		...
		if($this->table->getEnabledTransaction()) dibi::begin();
			...
			if(!$this->isMax) {
				...
				if($this->isParent) {
					...
					if($newColumn > $this->table->getMaxNumbered()) {
						throw new SakuraBadTargetException("Cannot insert new under choosed node!", 0, $this->table->getName(), $node, $whereTo);
					}

					$DibiFluent = $this->table->setNumberedUpdate($newColumn, $firstEmptyLevel, $movement = 1);
					$DibiFluent->set("`" . $this->table->getAlias() . "`.`" . $this->table->getNameOfNumbered() . "$count` = %i", 1);
					$DibiFluent = $this->table->setNumberedWhere($DibiFluent, $this->table->getMinNumbered(), $count, '=');

				} else {
					$DibiFluent = $this->table->setNumberedUpdate($count, $count, $movement = 0, $add = 1);
					$DibiFluent = $this->table->setNumberedWhere($DibiFluent, $this->table->getMinNumbered(), ($count - 1) );
					$DibiFluent = $this->table->setNumberedWhere($DibiFluent, $count, $count, '>=');
				}

				$DibiFluent->execute();
			}

			if($node instanceof DibiRow) $node = $node->toArray();

			if(isset($node[$this->id])) unset($node[$this->id]);
			...
			dibi::query('INSERT INTO `' . $this->table->getName() . '` %v', $node);
			if(dibi::affectedRows() != 1) throw new SakuraNotSupportedException('After inserting node a value of affectedRows() is not equal to 1!');
	...
	*/
	public function deleteNode () {
		...
		try {
			...
			$firstNull = $this->getPositionOfFirstNull($node);
			if($firstNull == 2) throw new SakuraNotSupportedException("Cannot delete root!");
			...
			if(!is_null($firstNull)) {
				...
				if($max > 1 and $lastNotNull == $min) throw new SakuraNotSupportedException("Cannot delete root!", 0, $this->table->getName());
				...
				if($max) {
					if($max > 1) {
						$max--;
						$move = dibi::command()->update("`" . $this->table->getName() . "` AS `$alias`");
					...
					if($endOfBranch >= $lastNotNull + 1) {
						$fromRightToLeft = dibi::command()->update("`" . $this->table->getName() . "` AS `$alias`");
						...
						$affectedRows = dibi::affectedRows();
					}

				} else {
					$back = dibi::command()->update("`" . $this->table->getName() . "` AS `$alias`");
	...
	*/
	public function updateBranch () {
		...
		try {
			...
			$whereTo = $this->getNodeWhereTo();
			$from = $this->getNode();
			if($from instanceof DibiRow) $from = $from->toArray();
			...
			if($allSame) throw new SakuraNotSupportedException("Cannot move superior node to its descendant!");

			if($this->isMax) $this->isMax = False; // reset to default
			else {
				$move = dibi::command()->update("`" . $this->table->getName() . "` AS `$alias`");
				...
				if($allSame and $whereTo[$lastNotNullWhereTo] <= $from[$name . $lastNotNullWhereTo]) $from[$name . $lastNotNullWhereTo]++;
			}
			...
			if($diff < 0) {
				...
				if($lastNotNullFrom == $max) {
					...
					if($endOfBranch == $lastNotNullFrom) $DF = NULL;
					else $DF = $this->table->setNumberedUpdate($firstNullWhereTo, $endOfBranch, $diff, $add);
				}
				...
				if($diff + $endOfBranch > $max) {
					throw new SakuraBadTargetException("Cannot move current branch under choosed node!", 0, $this->table->getName(), $from, $whereTo);
				}
			...
			$repair = dibi::command()->update("`" . $this->table->getName() . "` AS `$alias`");
	...
	/**
	* It returns generally set DibiFluent instance.
	* @return DibiFluent
	*/
	...
	/**
	* It returns DibiFluent instance for select tree.
	* @return DibiFluent
	*/
	...
	/**
	* It returns DibiFluent instance for select branch.
	* @return DibiFluent
	*/
	...
	public function selectPathSqlFactory ($includingNode, $fromRoot, $selectId = False) {
		...
		if($selectId) return dibi::select("`" . $this->table->getAlias() . "`.`{$this->id}`")->from('`' . $this->table->getName() . '` AS `' . $this->table->getAlias() . '`');
		else return dibi::select(False)->from('`' . $this->table->getName() . '` AS `' . $this->table->getAlias() . '`');
	}
	...
	public function getParent () {
		...
		throw new SakuraNotSupportedException("Cannot return 'parent' column. There is no 'parent' column in Level method.");
	}
